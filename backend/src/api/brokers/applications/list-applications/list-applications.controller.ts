...
export class BrokerApplicationsListController {
  ...
  async post(
    ...
    const is_above_avg = body.loanAmount >= avgLoanAmount ? true : false;
    console.log(body)
    console.log(body1)
    const application = await this.applicationEntity.create({ ...body,
      applicantName:body1.applicantName, 
      applicantEmail:body1.applicantEmail, 
      applicantMobilePhoneNumber:body1.applicantMobilePhoneNumber, 
      applicantAddress:body1.applicantAddress, 
      incomingAddress:body1.incomingAddress, 
      outgoingAddress:body1.outgoingAddress, 
      status: ApplicationStatus.Submitted, brokerId: user.id });
    return {
      success: true,
      loanAmount,
      is_above_avg
